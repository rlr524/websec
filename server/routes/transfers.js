...
router.get("/", csrfProtection, function (req, res /*, next*/) {
	bounceOutIfLoggedOut(req, res, () => {
		...
		Account.findAll({
			...
			let myAccounts = allAccounts.filter(
				(a) => a.userId === req.session.currentUser.id
			);
			let userAccounts = collection.groupBy(
				allAccounts.map((a) => a.get({ plain: true })),
				(acc) => acc.user.username
			);
			//////////////////////////////////////////////////////////////
			//// ↓ EXERCISE 5 SOLUTION GOES HERE
...
router.post("/perform", csrfProtection, function (req, res) {
	bounceOutIfLoggedOut(req, res, () => {
		let { accountFrom, accountTo, amount } = Object.assign(
		...
		if (!accountFrom || !accountTo) {
			errorAndReload(
				req,
				res,
				"Must specify accounts to transfer from and to"
			);
			return;
		}
		if (accountFrom === accountTo) {
			errorAndReload(
				req,
				res,
				"Cannot transfer funds from an account to its self"
			);
			return;
		}
		if (!amount || isNaN(amount) || amount <= 0) {
			errorAndReload(
				req,
				res,
				"Must specify a valid, positive amount to transfer"
			);
			return;
		}

		Promise.all(
			[accountFrom, accountTo].map((accountId) =>
				...
				return Db.transaction((t) => {
					return aFrom
						.update(
							{
								balance:
									Math.round(100 * (aFrom.balance - amount)) /
									100,
							},
							{ transaction: t }
						)
